...
async def hello(ctx):
    ...
    source = requests.get(f'https://api.popcat.xyz/fact').json()
    f=source['fact']
...
@client.command()
async def meme(ctx):
    d = requests.get(f'https://api.popcat.xyz/meme').json()
    title=d['title']
    img=d['image']
    url=d['url']
    like = d['upvotes']
...
async def joke(ctx): 
  ...
  page = requests.get('https://joke.deno.dev/').json()
  joke = page['setup']
  answer = page['punchline']
...
async def bot( ctx):
  ...
  embed.add_field(name="<:blurple_slashcommands:930349698999537746> Prefix", value=f"┕ {ctx.prefix}")
  embed.add_field(
...
async def goal(ctx):
  ...
  em = discord.Embed(title=f"Invite {client.user.name}",description=f"** {currentg}/{goal}**\n\n",color=discord.Color.gold())
  em.set_footer(text="Hiya , Invite me to your server and help me reach my goal!")
  em.set_thumbnail(url="https://img.freepik.com/free-vector/positive-tiny-people-sitting-walking-near-huge-target-with-arrow-isolated-flat-vector-illustration-cartoon-business-team-achieving-goal-aim-marketing-strategy-achievement-concept_74855-10139.jpg")
  #em.add_field(name="Goal 1(25 servers) " ,value=f"Achieved on 2nd Feb 2022",inline=False) 
  goals="""<:reply_cont:1002518635782344704> **1**  - 4/12/2021                  
           <:reply_cont:1002518635782344704> **25** - 2/02/2022                 
           <:Reply:1002518537371394099> **50** - 17/07/2022 **?** 
        """
  em.add_field(name="Server Goals",value=goals)
