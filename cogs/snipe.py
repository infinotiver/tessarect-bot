...
class Snipe(commands.Cog):
    ...
    @commands.is_owner()
    async def adminsnipe(self, ctx: commands.Context, limit: int = 1):
        if limit > self.adsnipe_limit:
            await ctx.send(f"Maximum snipe limit is {self.snipe_limit}")
            return
    ...
    @commands.command(
        name="deveditsnipe",
        aliases=["adesn"],
        help=" DEV See recently edited messages in the current channel",
        hidden=True,
    )

    async def admineditsnipe(self, ctx: commands.Context, limit: int = 1):
        if limit > self.adsnipe_limit:
            await ctx.send(f"Maximum snipe limit is {self.adsnipe_limit}")
            return
